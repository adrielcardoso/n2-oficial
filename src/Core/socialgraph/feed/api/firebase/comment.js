...
export const updateReactionsCountForFollowers = async (post) => {
  ...
  const unsubscribe = friendship.subscribeToInboundFriendships(
    post.authorID,
    (inboundFriendships) => {
...
const updateCommentCountOnAllFriendsTimelines = async (comment) => {
  ...
  const unsubscribeInbound = friendship.subscribeToInboundFriendships(
    comment.authorID,
    (inboundFriendships) => {
      const unsubscribeOutbound = friendship.subscribeToOutboundFriendships(
        comment.authorID,
        (outboundFriendships) => {
          const inboundUserIDs = inboundFriendships.map(
            (friendship) => friendship.id,
